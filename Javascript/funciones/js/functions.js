...
function dniLetter( dni ) {
	...
	var num = dni % 23;
//	var result = lockup.substring(num, num+1);
	var result = lockup[num];
	return result;
}
...
console.log( dniLetter( "999999 9" ) === 'Invalid parameter');
...
function findLongestWord( words_array ){
...
/*
array_lengths.push( findLongestWord( ["AngryDragon", "FlyMonster", "UltraMar", "CrazyHorseman", "BocaPozo"]) );
array_lengths.push( findLongestWord( ["Alpha", "Charlie", "Cafe", "IronHack", "Rocodromo"]) );
array_lengths.push( findLongestWord( ["Insidious", "LockSmith", "Brasas", "Cangrejito", "BlackPepperCrab"]) );
*/
...
/*
function calculateAverage( words_array )

{
	...
	while (index < words_array.length) {
		sum += words_array[index];
		index++;
	}

	average = sum / words_array.length; 
...
function charFreq( string ) {
	...
	string.split('').forEach(function(s){
		freqCounter[s] = freqCounter[s] ? freqCounter[s]+1 : 1;
	});
	return freqCounter;
}
